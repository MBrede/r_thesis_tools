...
tibble(n = c(2,6, 2, 3, 2),
  ...
  bind_rows(
    tibble(n = c(4 ,1, 3, 0, 2, 1),
              Strategie = c('"normale" Suchmaschinen', 'Google Scholar', 'Datenbanken', 'Lehr-/Handbücher', 'Übersichtsartikel', 'Sprachmodell fragen'),
              Kohorte = 2024)
 )|> 
   bind_rows(
    tibble(n = c(2 ,3, 5, 0, 0, 1),
              Strategie = c('"normale" Suchmaschinen', 'Google Scholar', 'Datenbanken', 'Lehr-/Handbücher', 'Übersichtsartikel', 'Sprachmodell fragen'),
              Kohorte = 2025)
 )|> 
  mutate(Strategie = factor(Strategie, levels = c('"normale" Suchmaschinen', 'Google Scholar','Lehr-/Handbücher','Datenbanken','Übersichtsartikel', 'Sprachmodell fragen'))) |> 
...
6. **Sprachmodell fragen**: Entweder der sehr fragwürdige Ansatz, sich Literatur von einem Sprachmodell auflisten zu lassen. ChatGPT, Gemini und Claude bieten jetzt auch einen Deep Research Modus an, der eher empfehlenswert ist - dazu später mehr.
...
* [scispace](https://scispace.com/) (Anmeldung nötig, freies Volumen, dann relativ kostspielig)

Außerdem bieten, wie oben schon erwähnt, Gemini, ChatGPT und Claude jetzt auch Features an, die "Deep Research" oder ähnlich heißen.
...
Ob und wie Ihr KI nutzen dürft, müsst Ihr mit Euren Betreuenden absprechen.
...
answers25 <- ("Google Scholar, PubMed, PsychINFO
Google Scholar, Hogrefe eContent 
Researchgate, PsychINFO, scopus
Google Scholar, PubMed
PubMed, Google Scholar, Researchgate, Web of Science
PsychINFO, Google Scholar
Google Scholar
Web of Science, PubMed, PsychINFO, Google Scholar, Researchgate, FIS Bildung, ERIC, Science Direct") |> 
  parse_mentions_list(2025)


p1 <- bind_rows(answers23, answers24, answers25) |> 
  count(Quellen, Kohorte) |> 
  mutate(
    Quellen = forcats::fct_lump_n(Quellen, 1, other_level = "Andere"),
    Quellen = forcats::fct_reorder(Quellen, n)) |> 
  double_cohort_plot(Quellen) +
    labs(x = '', y = 'Nennungen', caption = "Verteilung der genutzten Quellen bei der Suche nach Literatur") 

p2 <- bind_rows(answers23, answers24, answers25) |> 
  count(id, Kohorte) |> 
